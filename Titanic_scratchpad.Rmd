...
It would be also interesting to include gender differences when considering Age:
```{r}
ggplot(data = training, aes(x = Age, fill= Survived))+
...
It would be therefore sensible to impute Age by random gaussian imputation using the mean and standard deviation of individual SibSp levels. We will also include Sex in this model to account for gender-specific differences in Age as well as Pclass to account for the interesting seperation of Age by Passenger Class we noted above. In order to do this more rigorously, we can first fit a linear model with the existing Age , SibSp , Sex  and Pclass  data:

##### Age ~ SibSp + Sex + Pclass + e (random gaussian error)

```{r}
lmAge = lm(Age ~ SibSp+ Sex + Pclass, data = training, na.action = "na.omit")
summary(lmAge)
par(mfrow = c(2,2))
plot(lmAge)[1]
plot(lmAge)[2]
...
plot(lmAge)[4]
```
This model is just OK, but we don't need the perfect model for this type of imputation. Nice to see that both variance and normality assumptions of the model holds and allmost all levels of the covariates have significant impact on the mean outcome in the presence of each other. This would give us a good estimation for the missing values of Age.
